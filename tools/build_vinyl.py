...
function expRows(){return RECORDS.map((r,i)=>({artist:r.artist,title:r.title,year:r.year||'',genre:r.genre,status:r.status,condition:r.condition||'',favorite:FAVS[i]?'yes':'',value:(VALUES[i]!=null?VALUES[i]:''),note:r.note||''}));}
function dl(name,text,type){const b=new Blob([text],{type:type});const u=URL.createObjectURL(b);const a=document.createElement('a');a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1000);}
function expCSV(){const rows=expRows(),cols=Object.keys(rows[0]);const qq=v=>{v=String(v);return /[",\n]/.test(v)?'"'+v.replace(/"/g,'""')+'"':v;};dl('wax-stacks.csv',[cols.join(',')].concat(rows.map(r=>cols.map(c=>qq(r[c])).join(','))).join('\n'),'text/csv;charset=utf-8');}
function expJSON(){dl('wax-stacks.json',JSON.stringify(expRows(),null,2),'application/json');}
document.getElementById('expCSV').addEventListener('click',expCSV);
document.getElementById('expJSON').addEventListener('click',expJSON);
...
<div class="wrap">
  ...
  <div class="controls">
    ...
    <button id="expCSV" class="needlebtn alt">&#8681; CSV</button><button id="expJSON" class="needlebtn alt">&#8681; JSON</button>
  </div>
